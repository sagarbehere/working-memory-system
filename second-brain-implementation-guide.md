...
|---|---|---|
| `reminder` | Todoist | API call, create task with due date |
| `record` (`structured`) | SQLite `records` table | Insert row: `id, type, domain, occurred_at, entity, data_json, notes` |
| `record` (`narrative`) | Obsidian vault | Dated note, existing LLM Wiki skill |
| `project` | Obsidian vault | Note with `status:` frontmatter |
| `reference` | Obsidian vault | Note with `status:` + `subtype:` frontmatter (entity/concept/procedure) |
| `idea` | Obsidian vault | Atomic note, freely linked |
